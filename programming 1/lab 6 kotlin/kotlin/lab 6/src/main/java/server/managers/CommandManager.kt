 ...
 */
class CommandManager(cm: CollectionManager, private val server: Server) {
    ...
    fun execute(line: String, reader: BufferedReader) {
        ...
        if (command != null) {
            ...
            if (command is Promptable) {
                try {
                    server.sendResponse((command as Promptable).execute(reader, arg))
                } catch (e: IOException) {
                    server.sendResponse(
                        Response(
                            ResponseStatus.INTERMEDIATE_ERROR,
                            "Ошибка ввода-вывода: " + e.message
                        )
                    )
                } catch (e: NullPointerException) {
                    server.sendResponse(Response(ResponseStatus.INTERMEDIATE_ERROR, e.message))
                ...
                try {
                    val response = command.execute(arg)
                    response.status = ResponseStatus.INTERMEDIATE_SUCCESS
