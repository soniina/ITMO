...
class Person : Comparable<Person>, Serializable {
    var name: String? = null //Поле не может быть null, Строка не может быть пустой
        set(value) {
            if (value == null || value.trim().isEmpty()) throw NullPointerException("Person.name: Значение не может быть пустым!")
            else field = value
        }
    var birthday: LocalDateTime? = null //Поле не может быть null
    var eyeColor: Color? = null //Поле может быть null
    var nationality: Country? = null //Поле не может быть null
    var location: Location? = null //Поле не может быть null
        set(value) {
            if (value == null) throw NullPointerException("Person.location: Значение не может быть пустым!")
            else field = value
        }
    ...
    fun setEyeColor(color: String) {
        if (color.trim { it <= ' ' }.isEmpty()) {
            this.eyeColor = null
            return
        }
        for (value in Color.entries) {
            if (value.color == color.uppercase(Locale.getDefault()).replace('ё', 'е') || value.name == color.uppercase(
                    Locale.getDefault()
                )
            ) {
                this.eyeColor = value
    ...
    fun setNationality(country: String) {
        if (country.trim { it <= ' ' }
                .isEmpty()) throw NullPointerException("Person.nationality: Значение не может быть пустым!")
